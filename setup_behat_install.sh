...
# BranchId=master
# BehatConfigFile=behat_config_parallel.php.template
# DbName=${SiteId}
...
# pgsql="DbType:pgsql DbUser:postgres DbPwd:moodle DbHost:localhost"
...
#BranchId=master
#BehatConfigFile=behat_config_parallel.php.template
#dbtorun="pgsql mysql mariadb pgsql mysql pgsql pgsql"
#pgsql="DbType:pgsql DbUser:moodle DbPwd:moodle DbHost:172.21.0.3"
#PARALLELPROCESS=3
#SELENIUMPORT=5555
...
if [ ! -L "/var/www/html/screenshots_$SiteId" ]; then
    ln -s $faildumpdir /var/www/html/screenshots_$SiteId
fi
...
if [ -n "${SUITE_NAME}" ]; then
    php admin/tool/behat/cli/init.php -j=$PARALLELPROCESS --add-core-features-to-theme=${SUITE_NAME}
else
