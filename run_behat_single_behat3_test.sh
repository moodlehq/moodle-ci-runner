...
# Run tests.
cd $moodledir/$SiteId
echo "CMD: vendor/bin/behat --config $moodledatadir/behat_$SiteId/behatrun/behat/behat.yml --format $OutputFormat --out --profile $behatprofiletouse --verbose $TAGS"

vendor/bin/behat --config $moodledatadir/behat_$SiteId/behatrun/behat/behat.yml --format $OutputFormat --out std --profile $behatprofiletouse --verbose $TAGS --format junit --out $junitreportspath
exitcode=${PIPESTATUS[0]}

# Re-run failed scenarios, to ensure they are true fails.
if [ "${exitcode}" -ne 0 ]; then
    echo "---Running behat again for failed steps---"
    vendor/bin/behat --config $moodledatadir/behat_$SiteId/behatrun/behat/behat.yml --format $OutputFormat --out std --profile $behatprofiletouse --verbose --rerun # --out $junitreportspath
    exitcode=${PIPESTATUS[0]}
fi
