...
#dbtorun="pgsql pgsql pgsql pgsql pgsql mysql oracle"
...
if [[ -z ${profile} ]]; then
  command="sudo docker run --rm -v /store/faildumpi/php7:/shared moodle/php7:${branch} /scripts/behat.sh ${git}"
elif [[ ${profile} = 'chrome' ]]; then
  command="sudo docker run --privileged --rm -v /store/faildump/php7:/shared moodle/php7:${branch} /scripts/behat.sh ${git}"
else
  command="sudo docker run --rm -v /store/faildump/php7:/shared moodle/php7:${branch} /scripts/behat.sh ${git}"
fi
...
if [[ -z ${dbprefix} ]]; then
  behatdbprefix="--behatdbprefix=b{$run}_"
else
  behatdbprefix="--behatdbprefix=${dbprefix}"
fi
...
if [[ -n ${profile} ]]; then
  ...
  if [[ ${profile} = 'chrome' ]]; then
    command="${command} --tags=@javascript"
  fi
fi
if [[ -n ${format} ]]; then
