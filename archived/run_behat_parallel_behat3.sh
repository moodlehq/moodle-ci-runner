...
php admin/tool/behat/cli/run.php --format="$OutputFormat" --out=std --profile=firefox{runprocess} --replace="{runprocess}" #--format="junit" --out="$junitreportspath"
...
if [ "${exitcode}" -ne 0 ]; then
    ...
    for ((i=1;i<=$PARALLELPROCESS;i+=1)); do
	    ...
	    sleep 5
    	    ...
    	    vendor/bin/behat --config $moodledatadir/behat_$SiteId/behatrun${i}/behat/behat.yml --format $OutputFormat --out std --verbose --rerun --profile "firefox${i}"
