...
if [[ -z ${profile} ]]; then
  command="sudo docker run --rm -v /store/faildump:/shared moodle/moodle:MOODLE_29_STABLE /scripts/behat.sh ${git}"
elif [[ ${profile} = 'chrome' ]]; then
  command="sudo docker run --privileged --rm -v /store/faildump:/shared moodle/moodle:MOODLE_29_STABLE /scripts/behat.sh ${git}"
else
  command="sudo docker run --rm -v /store/faildump:/shared moodle/moodle:MOODLE_29_STABLE /scripts/behat.sh ${git}"
fi

if [[ -z ${branch} ]]; then
  branch="--branch=MOODLE_29_STABLE"
else
