...
docker run --rm moodle/php7:${BranchId} /scripts/phpunit.sh --dbtype=${DbType} --dbhost=${DbHost} --dbname=${DbName} --branch=${BranchId} --phpunitdbprefix=${PhpunitDbPrefix} --dbuser=${DbUser} --dbpass=${DbPwd}
