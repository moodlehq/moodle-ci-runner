...
function moodle-core-copy_setup() {
    ...
    if [[ -n "$FULLGIT" ]] && [[ -d "${CODEDIR}/.git" ]]; then
        ...
        # need any credential at all, same as https://. Also reject any URL carrying userinfo
        # (e.g. https://user:token@host/repo.git), because that's not credential-free any more.
        remoteurl=$(docker exec -u www-data "${WEBSERVER}" git config --get remote.origin.url) || remoteurl=""
        if [[ ! "${remoteurl}" =~ ^(https|git)://[^@/]+(/|$) ]]; then
            exit_error "Only https:// and git:// public repositories, without embedded credentials," \
                "are supported for this job, not ssh+git ones. Found: ${remoteurl:-none}"
        fi
